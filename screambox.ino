...
/* using 0 as input of the PIR sensor
 * using 1 as output to signal motors 
 */
int pir_pin=TRIG1;
int motor_signal_pin=TRIG2;
...
const int STATE_START_PLAYING=1;
const int STATE_PLAYING=2;
const int STATE_STOP_PLAYING=3;

void start_mp3()
{
  int result;
  
  Serial.println("start_mp3()");
  result = MP3player.playMP3("6.mp3");
  Serial.print("result=");
  Serial.print(result);
  Serial.println("\n");
...
void loop()
{
    ...
    switch(state) {
        ...
        case STATE_PLAYING : 
            /* don't change state until the mp3 is finished */
            if( !MP3player.isPlaying() ) {
                state = STATE_STOP_PLAYING;
            }
            else { 
                delay(1000);
            }
